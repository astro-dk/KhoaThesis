...
\begin{eqnarray}
\frac{d P(r)}{dr}&=& -\frac{G \rho(r)\mathcal{M}(r)}{r^2} 
...
mass distribution from the spherical shape at multipole order $\ell$), and $S_L$ is the 
...
and the tidal multipole moments of NS. The higher $\lambda_\ell$ (or $\sigma_\ell$) is, 
...
\citep{perot2021role}
\begin{IEEEeqnarray*}{rCl}
    H''_\ell(r) &+& H'_\ell(r) \left[ 1-\frac{2G\mathcal{M}(r)}{c^2 r}  \right]^{-1} \left\{ \frac{2}{r} - \frac{2G\mathcal{M}(r)}{c^2 r^2} - \frac{4\pi G}{c^4} r[\rho(r)c^2 - P(r)] \right\}\\
                &+& H_\ell(r) \left[ 1-\frac{2G\mathcal{M}(r)}{c^2 r} \right]^{-1} \Bigg\{ \frac{4\pi G}{c^4} \left[ 5\rho(r)c^2 + 9P(r) + c^2 \dv{\rho}{P}\left[ \rho(r)c^2 + P(r) \right] \right] \\
                &-& \frac{\ell(\ell+1)}{r^2} - 4 \left[ 1-\frac{2G\mathcal{M}(r)}{c^2 r} \right]^{-1} \left[ \frac{G\mathcal{M}(r)}{c^2 r^2} + \frac{4\pi G}{c^4} rP(r) \right]^2 \Bigg\} = 0\IEEEyesnumber
\end{IEEEeqnarray*}
for \gls{GE} perturbations and
\begin{IEEEeqnarray*}{rCl}
    \tilde{H}''_\ell(r) &-& \tilde{H}'_\ell(r) \left[ 1-\frac{2G\mathcal{M}(r)}{c^2 r} \right]^{-1} \frac{4\pi G}{c^4} r \left[ P(r) + \rho(r)c^2 \right]\\
                        &-& \tilde{H}_\ell(r) \left[ 1-\frac{2G\mathcal{M}(r)}{c^2 r} \right]^{-1} \left\{ \frac{\ell(\ell+1)}{r^2} - \frac{4G\mathcal{M}(r)}{c^2 r^3} + \theta \frac{8\pi G}{c^4} \left[ P(r) + \rho(r)c^2 \right] \right\} = 0\\\IEEEyesnumber
\end{IEEEeqnarray*}
...
as derived by \cite{perot2021role}. It is worth mentioning that similar to the case
of a charged fluid drop being deformed under electromagnetic field, the surface (or the
constant pressure level surface) of a \gls{NS} made of perfect fluid also deviates from its
supposedly spherical shape in the \gls{GE} perturbation. Indeed, this was shown in the
study of \cite{damour2009relativistic} through the shape Love numbers (or ``first type''
Love number, whereas $k_\ell$ is often referred to as the ``second type'')
$h_\ell$ where their relation with the \gls{GE} tidal Love numbers $k_\ell$ in the
Newtonian theory simply
\begin{equation}
    h_\ell = 1 + 2k_\ell.
\end{equation}
In the relativistic theory, on the other hand, the connection between them is much more
subtle and its functional form has yet been deduced explicitly \citep{damour2009relativistic}.
